...
class Slider(InstructionGroup):
    def __init__(self, initial_val, image_pos, image_size):
        ...
        self.line = Line(points=[image_pos[0], image_pos[1], image_pos[0] + image_size[0], image_pos[1]], width=10)
        self.add(self.line)

        dim_size = min(0.03*Window.width, 0.03*Window.height)
        self.add(Color(rgb=(136/255, 195/255, 0)))
        self.slider_knob = CEllipse(cpos=(image_pos[0] + self.value*image_size[0], image_pos[1]), csize=(dim_size, dim_size))
        self.knob_selected = False
        self.add(self.slider_knob)


    def on_update(self):
        self.slider_knob.cpos = (self.image_pos[0] + self.value*self.image_size[0], self.image_pos[1])
